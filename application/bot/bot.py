...
class Bot:
    def __init__(self, token: str, group_id: int, key_word_end="Выход"):
        ...
        self.long_poller: VkBotLongPoll = VkBotLongPoll(
            vk=self.vk, group_id=self.group_id
        )
    ...
    def check_answer_user(
        self, text_user: str, user_id: int, step_user: typing.Dict[str, str]
    ) -> bool:
        """
        ...
        if step_user["handler_user_text"] is None:
            return True
        handler_user_text: typing.Callable = getattr(
            handler_user, step_user["handler_user_text"]
        )
        return handler_user_text(text_user, user_id)

    def create_keyboard(
        self, state_user: UserState, settings_keyboard: typing.Dict[str, typing.Any]
    ) -> Keyboard:
        """
        Создаем клавиатуру
        """
        if settings_keyboard:
            get_items_keyboard: typing.Callable = getattr(
                model_handlers, settings_keyboard["items_handler"]
            )
            if get_items_keyboard:
                items: typing.List[str] = get_items_keyboard(state_user.context)
                keyboard_obj: Keyboard = Keyboard(
                    items=items,
    ...
    def continue_scenario(self, text_user: str, user_id: int) -> None:
        ...
        """
        state_user: UserState = user_model.get_state_user(user_id)
        steps: typing.Dict[str, typing.Any] = settings.SCENARIOS[
            state_user.scenario_name
        ]["steps"]
        ...
        if not self.express_end_scenario(text_user, user_id):
            if self.check_answer_user(text_user, user_id, step_user):
                step_user: typing.Dict[str, typing.Any] = steps[state_user.step_name]
                keyboard: Keyboard = self.create_keyboard(
                    state_user, step_user["keyboard"]
                )
    ...
    def send_response(
        self, user_id: int, step_user: typing.Dict[str, typing.Any], keyboard: Keyboard
    ) -> None:
        """
